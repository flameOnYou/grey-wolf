...
export default class CandleStickView extends TechnicalIndicatorView {
   ...
   */
  _drawRealTime () {
    const timeLinePoints = new Array()
    const timeLineAreaPoints = new Array()
    const averageLinePoints = new Array()
    ...
    const onDrawing = (x: number, i: number, kLineData: { close: number }, halfBarSpace: number) => {
  ...
  _drawCandleStick () {
    ...
    const onDrawing = (x: number, i: any, kLineData: any, halfBarSpace: number, barSpace: number) => {
    ...
    this._drawGraphics(onDrawing,null)
   ...
   */
  _drawLowestHighestPriceMark (priceMark: { color: string | CanvasGradient | CanvasPattern; textSize: number | undefined; textWeight: string | undefined; textFamily: string | undefined; textMargin: any }, x: any, price: number, isHigh: boolean, pricePrecision: number | undefined) {
