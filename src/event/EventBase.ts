...
function checkTouchEvents () {
  ...
  let win: any = window
  return Boolean(win.DocumentTouch && document instanceof win.DocumentTouch)
}
...
export default class EventBase {
    ...
    _clickCount = 0
    _clickTimeoutId :any
    _longTapTimeoutId :any
    _longTapActive = false
    _mouseMoveStartPosition :any
    _moveExceededManhattanDistance = false
    _cancelClick = false
    _unsubscribeOutsideEvents :any
    _unsubscribeMousemove :any
    _unsubscribeRoot :any

    _startPinchMiddlePoint:any
    _startPinchDistance = 0
    _pinchPrevented = false
    _preventDragProcess = false

    _mousePressed = false


  constructor (target:any, eventHandler:any, options:any) {
    this._target = target
  ...
  _mouseEnterHandler (enterEvent:any) {
  ...
  _mouseMoveHandler (moveEvent:any) {
  ...
  _mouseWheelHandler (wheelEvent:any) {
  ...
  _mouseMoveWithDownHandler (moveEvent:any) {
  ...
  _mouseUpHandler (mouseUpEvent:any) {
  ...
  _mouseDownHandler (downEvent:any) {
  ...
  _initPinch () {
    ...
    this._target.addEventListener(
      'touchstart',
      (event:any) => this._checkPinchState(event.touches),
      { passive: true }
    )
    ...
    this._target.addEventListener('touchend', (event:any) => {
      this._checkPinchState(event.touches)
    })
  }

  _checkPinchState (touches:any) {
    if (touches.length === 1) {
      this._pinchPrevented = false
    }
  ...
  _processEvent (event:any, callback:any) {
    if (!callback) {
      return
    }
