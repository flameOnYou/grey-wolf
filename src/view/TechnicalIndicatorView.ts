...
export default class TechnicalIndicatorView extends View {
  ...
  _drawTechnicalIndicator () {
    ...
    this._drawGraphics(
      (x: number, i: number, kLineData: any, halfBarSpace: number, barSpace: any) => {
   ...
   * @private
   */
  _drawGraphics (onDrawing:any, onDrawEnd:any) {
    const dataList = this._chartData.dataList()
   ...
   * @private
   */
  _drawCandleStickBar (x:number, halfBarSpace:number, barSpace:number, kLineData:Bar, barOptions:any, barStyle:any) {
