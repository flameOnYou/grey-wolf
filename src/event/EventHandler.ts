...
export function isMouse (event:any) {
  return event.type === EventType.MOUSE
}

export default class EventHandler {
  _chartContentSize:any = {}
  _paneContentSize:any = {}
  constructor (public _chartData:ChartData) {
    
  }

  _checkEventPointX (x:number) {
    return x > 0 && x < this._chartContentSize.contentRight - this._chartContentSize.contentLeft
  }

  setChartContentSize (chartContentSize:any) {
    this._chartContentSize = chartContentSize
  }

  setPaneContentSize (paneContentSize:any) {
