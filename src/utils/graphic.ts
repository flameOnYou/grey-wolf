 ...
 * @param targetPoints
 */
export function getLinearY (point1:Point, point2:Point, targetPoints:Array<Point>) {
  const v:number[] = []
 ...
 * @param targetPoint
 */
export function checkPointOnStraightLine (point1:Point, point2:Point, targetPoint:Point) :Boolean{ 
 ...
 * @returns {boolean}
 */
export function checkPointOnRayLine (point1:Point, point2:Point, targetPoint:Point) :Boolean{
  if (!targetPoint || !point1 || !point2) {
    return false
  }
 ...
 * @param targetPoint
 */
export function checkPointOnSegmentLine (point1:Point, point2:Point, targetPoint:Point):Boolean {
  if (!targetPoint || !point1 || !point2) {
    return false
  }
  if (checkPointOnStraightLine(point1, point2, targetPoint)) {
 ...
 * @returns {boolean}
 */
export function checkPointOnCircle (circleCenterPoint:Point, radius:number, targetPoint:Point) :Boolean{
  if (!targetPoint) {
    return false
  }
 ...
 * @returns {Array}
 */
export function getParallelLines (points:Array<Point>, size:any, isPriceChannelLine:Boolean) {
  const lines = []
  if (points.length > 1) {
 ...
 * @param size
 */
export function getFibonacciLines (points:Array<Point>, size:any) {
  const lines = []
  if (points.length > 0) {
    const startX = 0
    const endX = size.width
