...
export default class SeparatorPane {
    ...
    _dragEvent!:EventBase
    _startY!:number
  constructor (
    public _container:HTMLElement, 
    public _chartData:ChartData,
    public _paneIndex:number,
    public dragEnabled:boolean,
    public _dragEventHandler:any) {
    
    this._initElement(_container, dragEnabled)
  }

  _initElement (container:HTMLElement, dragEnabled:boolean) {
  ...
  _mouseDownEvent (event:MouseEvent) {
    this._startY = event.pageY
    this._dragEventHandler.startDrag(this._paneIndex)
  }

  _pressedMouseMoveEvent (event:MouseEvent) {
   ...
   * @param width
   */
  setSize (offsetLeft:number, width:number) {
   ...
   * @param paneIndex
   */
  updatePaneIndex (paneIndex:number) {
